...
def setup_image_embeddings_service(
    ...
    """Setup image embeddings service for multimodal features.
    
    Args:
        skip_image_embeddings: If True, skip creating image embeddings service to avoid 
                              Vision API rate limits. Images will still be described by GPT-4o.
    """
    import os
    
    # Check environment variable to skip image embeddings (avoids Vision API rate limits)
    # This is useful when RAG_SEARCH_IMAGE_EMBEDDINGS=false
    skip_from_env = os.getenv("RAG_SEARCH_IMAGE_EMBEDDINGS", "true").lower() == "false"
    if skip_image_embeddings or skip_from_env:
        return None
