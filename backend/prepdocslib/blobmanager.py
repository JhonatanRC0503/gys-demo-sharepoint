...
class BlobManager(BaseBlobManager):
    ...
    async def upload_document_image(
        ...
        # Note: user_oid is ignored for BlobManager (shared storage, no per-user separation)
        # If per-user storage is needed, use AdlsBlobManager instead
    ...
    async def download_blob(
        ...
        logger.info(f"[DEBUG] Attempting to download blob: '{blob_path}' from container: '{self.container}'")
        try:
