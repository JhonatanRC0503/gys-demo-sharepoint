...
class OpenAIEmbeddings(ABC):
    ...
    def split_text_into_batches(self, texts: list[str]) -> list[EmbeddingBatch]:
        ...
        for text in texts:
            ...
            # Truncate individual chunks that exceed the per-item API limit.
            # This can happen with large figure descriptions from multimodal processing.
            # The per-item limit equals batch_token_limit (8100), which is safely
            # below the Azure OpenAI hard cap of 8192 tokens per input item.
            if text_token_length > batch_token_limit:
                logger.warning(
                    "Text chunk of %d tokens exceeds the embedding per-item limit of %d tokens. "
                    "Truncating to fit. This may affect retrieval quality for large figure descriptions.",
                    text_token_length,
                    batch_token_limit,
                )
                try:
                ...
                text = encoding.decode(tokens[:batch_token_limit])
                text_token_length = batch_token_limit
