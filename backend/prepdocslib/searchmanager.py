        ...
        async with self.search_info.create_search_index_client() as search_index_client:
            ...
            if self.search_images:
                ...
                if self.search_info.azure_vision_endpoint and not skip_image_embeddings:
                    ...
                    # Images field WITH embedding for vector search
                    ...
                    logger.info("RAG_SEARCH_IMAGE_EMBEDDINGS FALSE")
                    images_field = SearchField(
                        name="images",
                        type=SearchFieldDataType.Collection(SearchFieldDataType.ComplexType),
                        fields=[
            ...
            if self.search_info.index_name not in [name async for name in search_index_client.list_index_names()]:
                ...
                if images_field:
                    logger.info("Including %s field for image descriptions in new index", images_field.name)
                    fields.append(images_field)
                    # Only add vector search components if we have image embeddings enabled
                    if image_vector_search_profile is not None and image_vector_algorithm is not None:
                ...
                if (
                    ...
                    logger.info("Adding %s field for image descriptions", images_field.name)
                    # Only set stored=True for the first field if it exists and has embedding
                    if len(images_field.fields) > 0 and hasattr(images_field.fields[0], 'stored'):
                        images_field.fields[0].stored = True
                    existing_index.fields.append(images_field)
                    
                    # Only add vector search components if image embeddings are enabled
                    if image_vector_search_profile is not None and image_vector_algorithm is not None:
                        if existing_index.vector_search is None:
                ...
                for section_index, section in enumerate(batch):
                    ...
                    if self.search_images:
                        ...
                        for image in section.chunk.images:
                            ...
                            # Only include embedding if it's not None (avoid null value errors)
                            if image.embedding is not None:
                                img_dict["embedding"] = image.embedding
                            image_data.append(img_dict)
                        image_fields = {"images": image_data}
                    document = {
